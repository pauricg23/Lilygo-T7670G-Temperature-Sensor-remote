...
void modemToLowPower() {
  // Let modem idle into PSM/minimum RF
  sendATCommand("AT+CFUN=0", 1200);   // minimum functionality
...
void setup() {
  ...
  if (wakeup_reason == ESP_SLEEP_WAKEUP_TIMER) {
    ...
    // Don't show battery here (modem isn't up yet); just turn the OLED off to save power.
    display.ssd1306_command(SSD1306_DISPLAYOFF);
    ...
    const bool allowProbe = !probeAlreadyDone();   // run probe once, ever
    ...
    if (allowProbe) {
      ...
      unsigned long until = millis() + 15000UL; // 15s probe window
      while (millis() < until) {
        ...
        if (!lteReady) {
          ...
          // LTE is up; show battery status (only read once per probe cycle)
          static bool batteryRead = false;
          static PowerState ps;
          if (!batteryRead) {
            ps = readPowerState();  // uses AT+CBC - only call once
            batteryRead = true;
          }
          display.print("Bat: "); display.print(ps.percent); display.print("% "); display.print(ps.vbat, 2); display.print("V");
        }

        // Larger temperature display to fill the extra space
        display.setTextSize(2);
        display.setCursor(2, 16); display.print("T1: "); display.print(t1,2); display.print("C");
        display.setCursor(2, 36); display.print("T2: "); display.print(t2,2); display.print("C");
        display.setCursor(2, 56); display.print("T3: "); display.print(t3,2); display.print("C");
        display.setTextSize(1); // Reset text size for next iteration
        display.display();
  ...
  if (!lteConnected) {
    Serial.println("❌ LTE setup failed, going to sleep");
    displayGoingToSleep();
    powerDownModem();
    enterDeepSleepSeconds(3600); // Sleep for 1 hour on failure
    return;
  }
